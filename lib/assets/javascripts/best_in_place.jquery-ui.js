...
BestInPlaceEditor.forms.date = {
    activateForm: function () {
      ...
      var timesheetTimezone = $(this.element).parents('.single_timesheet_report').data('timesheet-timezone');
      var defaultSettings = {
        onClose: function () { that.update(); }
      };
      if (rawDateValue !== ''){
        var dateValue = moment(Date.parse(rawDateValue));
        defaultSettings.hour = dateValue.tz(timesheetTimezone).hours();
        defaultSettings.minute = dateValue.tz(timesheetTimezone).minutes();
        defaultSettings.timezone = dateValue.utcOffset();
      };
